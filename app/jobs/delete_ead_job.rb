...
class DeleteEadJob < ApplicationJob
  ...
  # @param skip_delete_safeguard [Boolean] if true, the job will not raise an error if the number of records
  ...
  def perform(repository_id:, aspace_config_set:, skip_delete_safeguard: false)
    ...
    delete_safeguard unless skip_delete_safeguard

    delete_and_commit_to_solr(ids_to_delete)
  end

  def delete_safeguard
    DeleteSafeguard.check(delete_count: ids_to_delete.count, index_count: indexed_eads.count)
  end
