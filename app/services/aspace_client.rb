...
class AspaceClient
  ...
  def resource_description(resource_uri)
    ...
    resource_uri['resources'] = 'resource_descriptions'
    authenticated_get("#{resource_uri}.xml?include_daos=true")
  end

  # Returns an instance of AspaceQuery that response to :each returning
  # hashes containting an ead id and uri for resources of interest,
  ...
  def published_resource_uris(repository_id:, updated_after: nil)
    unless repository_id
      raise ArgumentError,
            'Please provide the ArchivesSpace repository id'
    end
