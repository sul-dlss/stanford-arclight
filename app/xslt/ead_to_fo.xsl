...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:ead="urn:isbn:1-931666-22-9" xmlns:fo="http://www.w3.org/1999/XSL/Format" version="2.0"  exclude-result-prefixes="local xlink ns2 ead xsl">
    <!-- Adapted from ArchivesSpace, https://github.com/archivesspace/archivesspace -->
    <!--
        *******************************************************************
        *                                                                 *
        * VERSION:          1.0                                           *
    ...
    <xsl:include href="ead_to_fo_helper_functions.xsl"/>
    ...
    <xsl:attribute-set name="inventoryHeader">
        <xsl:attribute name="font-size">14pt</xsl:attribute>
        <xsl:attribute name="font-weight">bold</xsl:attribute>
        <xsl:attribute name="border-top">4pt solid #333</xsl:attribute>
        <xsl:attribute name="margin-bottom">12pt</xsl:attribute>
    ...
    <xsl:template match="/">
        <fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format" font-size="12pt" font-family="KurintoText,KurintoTextJP,KurintoTextKR,KurintoTextSC,NotoSerif">
            ...
            <!-- Accessibility: Set the PDF title -->
            <fo:declarations>
    ...
    <xsl:template name="toc">
        <fo:block font-size="11pt" margin-top="12pt" margin-bottom="18pt">
            <fo:basic-link text-decoration="none" internal-destination="toc" color="#0D6CB6">Return to Table of Contents</fo:basic-link>
        </fo:block>
    </xsl:template>
    ...
    <xsl:template name="icon">
      <fo:block-container max-width="4in" max-height="3in">
        <fo:block text-align="center">
            <fo:external-graphic content-width="scale-down-to-fit"
              content-height="scale-down-to-fit" width="100%" height="100%"
              scaling="uniform" fox:alt-text="Stanford Unversity Libraries Logo">
            ...
            <fo:block xsl:use-attribute-sets="section">
                <xsl:if test="ead:did">
                    <fo:block text-align-last="justify">
                        <fo:basic-link internal-destination="{local:buildID(ead:did)}" fox:alt-text="{local:tagName(ead:did)}"><xsl:value-of select="local:tagName(ead:did)"/></fo:basic-link>
                        <xsl:text>&#160;&#160;</xsl:text>
    ...
    <xsl:template match="ead:archdesc/ead:did">
        ...
        <fo:block xsl:use-attribute-sets="h2ID">Summary Information</fo:block>
            ...
            <fo:table table-layout="fixed" width="100%">
                ...
                <!-- Accessibility: Tables need headers -->
                <fo:table-header>
                    <fo:table-row>
                        <fo:table-cell padding-bottom="8pt" padding-right="16pt" text-align="center" font-weight="bold">
                            <fo:block>Summary Category</fo:block>
                        </fo:table-cell>
                        <fo:table-cell padding-bottom="8pt" padding-right="16pt" text-align="center" font-weight="bold">
    ...
    <xsl:template match="ead:chronlist">
        ...
        <fo:table>
            ...
            <fo:table-header>
                <xsl:choose>
                    <!-- Check if there is content in ead:listhead -->
                    <xsl:when test="ead:listhead/node()">
                        <xsl:apply-templates select="ead:listhead"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <!-- Otherwise, provide default headers -->
                        <fo:table-row xsl:use-attribute-sets="th">
                            <fo:table-cell xsl:use-attribute-sets="tdBorder">
                                <fo:block xsl:use-attribute-sets="smp">Date</fo:block>
                            </fo:table-cell>
                            <fo:table-cell xsl:use-attribute-sets="tdBorder">
                                <fo:block xsl:use-attribute-sets="smp">Description</fo:block>
                            </fo:table-cell>
                        </fo:table-row>
                    </xsl:otherwise>
                </xsl:choose>
    ...
    <xsl:template match="ead:chronlist/ead:head">
        <fo:block xsl:use-attribute-sets="h3" font-size="14pt" margin-top="30pt" text-align="center">
            <xsl:apply-templates/>
        </fo:block>
    </xsl:template>
   ...
   <!-- Linking elements -->
    ...
    <!-- Collection Inventory Table Creation -->
            ...
            <xsl:choose>
                ...
                <xsl:when test="@level='subcollection' or @level='subgrp' or @level='series'
                    ...
                    @level='recordgrp' or @level='subfonds' or @level='class' or (@level='otherlevel' and not(child::ead:did/ead:container))">
                        <fo:block-container margin-left="{$clevelMargin}" background-color="#f0f0f0" border-top="3pt solid #ccc" keep-with-next.within-page="always">
                ...
                <xsl:when test="ead:did/ead:container[@label]">
                    <fo:block margin-left="{$clevelMargin}" border-top="1px solid #ccc" width="7in" padding-top="4pt">
                            <fo:block keep-with-next.within-page="always" margin-right="3in">
                                <xsl:apply-templates select="ead:did" mode="dscUnittitle"/>
                            </fo:block>
                        <fo:block-container margin-top="-14pt">
                            <fo:block text-align="right" keep-with-next.within-page="always" margin-left="4in">
                                <xsl:for-each-group select="ead:did/ead:container" group-starting-with=".[@label]">
                                    <xsl:apply-templates select="current-group()"/>
                                </xsl:for-each-group>
                            </fo:block>
                        </fo:block-container>
                        <fo:block-container margin-left="{$clevelMargin}">
                ...
                <xsl:otherwise>
                    <fo:block margin-left="{$clevelMargin}" border-top="1px solid #ccc" width="7in" padding-top="4pt">
                        ...
                        <fo:block-container margin-top="-14pt">
                            <fo:block text-align="right" keep-with-next.within-page="always" margin-left="4in">
                                <fo:inline><xsl:value-of select="ead:did/ead:container[1]/@type"/><xsl:text> </xsl:text><xsl:value-of select="ead:did/ead:container[1]"/></fo:inline>
                                <fo:inline><xsl:value-of select="ead:did/ead:container[2]/@type"/><xsl:text> </xsl:text><xsl:value-of select="ead:did/ead:container[2]"/></fo:inline>
                                <fo:inline><xsl:value-of select="ead:did/ead:container[3]/@type"/><xsl:text> </xsl:text><xsl:value-of select="ead:did/ead:container[3]"/></fo:inline>
    ...
    <xsl:template match="ead:container">
            <fo:inline margin="4pt 0"><xsl:value-of select="@type"/><xsl:text> </xsl:text><xsl:value-of select="."/></fo:inline>
    </xsl:template>
    ...
    <xsl:template match="ead:did" mode="dscUnittitle">
        <fo:inline margin-bottom="4pt" margin-top="4pt">
