...
RSpec.describe SemanticSearch::EmbeddingService do
  ...
  around do |example|
    ...
    # Most examples here exercise the OpenAI-compatible transport, which is the
    # Vertex models' route. The DEFAULT model (gemini-embedding-2) deliberately
    # does NOT use it - see the 'gemini native transport' describe, which clears
  ...
  describe 'model + task_type configuration' do
    def captured_body
      body = nil
      stub_request(:post, embed_url).with { |req| body = JSON.parse(req.body); true }
                                    .to_return(status: 200, body: response_body([Array.new(768, 0.1)]))
  ...
  # The default model (gemini-embedding-2) does NOT use the OpenAI route. Sending
  # it there would drop taskType and outputDimensionality, silently producing a
  # different vector space from the one gemini was evaluated in - so these pin the
  # transport, not just the body.
  describe 'gemini native transport' do
    # ?key= is part of the match: the pass-through authenticates by query param.
    let(:gemini_url) do
      'https://gateway.test/gemini/v1beta/models/gemini-embedding-2:batchEmbedContents?key=test-key'
    end

    # Clear the OpenAI-transport override the outer `around` sets, so these run
    # against the real configured default.
    before { ENV.delete('SEMANTIC_SEARCH_EMBEDDING_MODEL') }

    it 'routes the default model to the pass-through, not the OpenAI endpoint' do
      openai = stub_request(:post, embed_url)
      stub_request(:post, gemini_url)
        .to_return(status: 200, body: { embeddings: [{ values: Array.new(768, 0.1) }] }.to_json)

      expect(SemanticSearch.embedding_model).to eq('gemini-embedding-2')
      expect(service.embed('ships').length).to eq(768)
      expect(openai).not_to have_been_requested
    end

    it 'posts batchEmbedContents with taskType and outputDimensionality' do
      captured = nil
      stub_request(:post, gemini_url)
        .with { |req| captured = JSON.parse(req.body) } # truthy hash == a match
        .to_return(status: 200, body: { embeddings: [{ values: Array.new(768, 0.1) }] }.to_json)

      service.embed('ships')

      request = captured.fetch('requests').first
      expect(request['model']).to eq('models/gemini-embedding-2')
      expect(request['content']).to eq('parts' => [{ 'text' => 'ships' }])
      expect(request['taskType']).to eq('RETRIEVAL_QUERY')
      expect(request['outputDimensionality']).to eq(768)
    end

    it 'authenticates with a key query param rather than a bearer header' do
      stub = stub_request(:post, gemini_url)
    ...
    it 'returns unit vectors in request order' do
      stub_request(:post, gemini_url)
        .to_return(status: 200, body: { embeddings: [{ values: one_hot(0) }, { values: one_hot(1) }] }.to_json)

      vecs = service.embed_batch(%w[a b])
      expect(vecs.map { |v| v.index(1.0) }).to eq([0, 1])
    end

    it 'raises rather than mis-aligning when the response is short a row' do
      stub_request(:post, gemini_url)
        .to_return(status: 200, body: { embeddings: [{ values: Array.new(768, 0.1) }] }.to_json)

      expect { service.embed_batch(%w[a b]) }
        .to raise_error(SemanticSearch::EmbeddingService::ApiError, /expected 2 embeddings, got 1/)
    end

    it 'surfaces a 429 as a retryable rate-limit error' do
      stub_request(:post, gemini_url).to_return(status: 429, body: 'slow down')
      expect { service.embed('ships') }.to raise_error(SemanticSearch::EmbeddingService::RateLimitError)
    end
  end
