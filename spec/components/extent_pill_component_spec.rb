...
RSpec.describe ExtentPillComponent, type: :component do
  ...
  let(:document) do
    SolrDocument.new(id: 'abc123',
                     extent_ssm: ['1 Box', 'This is a rather long extent so we should truncate it'])
  end
  let(:compact) { false }
  ...
  it 'renders the extents' do
    ...
    expect(page).to have_text 'This is a rather long extent so we should ...'
  end
  ...
  context 'when compact is true' do
    ...
    it 'does not render the extents' do
      ...
      expect(page).to have_no_text '2 Feet'
    end
  end
end
